...
1. How many features does each layer contain and what kind of geometry are they?
 - *forests* has 32 features
 - *parks* has 47 features
 - *playgrounds* has 51 features
 - *shelters* has 19 features
 - forests and parks are MULTIPOLYGON. PLaygrounds and shelters are POINTS. 

2. What is the CRS value in these objects?
 - *forests*: ETRS89/ UTM zone 32N
 - *parks*: ETRS89/ UTM zone 32N
 - *playgrounds*: NA (we need to define this CRS later in the .Rmd)
 - *shelters*: ETRS89/ UTM zone 32N
...
# Use filter() to limit to over 5 ha parks (which is 50.000 m2)
...
# Limit the variables to "navn" and "areal_m2" variables
...
# This will not work
result <- st_area(parks) # this works
result > 100000 " but this doesn't"
```
...
class(elevation) #it's a "RasterLayer" and "raster
class(aarhus) #it's a "RasterBrick" and "raster

# Identify how many layers each object has
nlayers(elevation) # 1 layers
nlayers(aarhus) # 5 layers

#rows and columns of "elevation" raster
nrow(elevation)
ncol(elevation)

#rows and columns of "aarhus" raster
nrow(aarhus)
ncol(aarhus)
...
1. What are the dimensions (number of columns and rows) of the `elevation` raster?
  - `elevation` raster has 396 rows and 876 columns
  - `aarhus` raster has 3334 rows and 3334 columns
  
2. What is the resolution (opløsning) of `aarhus` orthophoto? How many layers does it contain and what do they represent?
  - the resolution is "1, 1  (x, y)"
  - it contains 5 layers, which represents the three color channels + length and width of the picture?
...
crs(aarhus) #it's +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs 
crs(elevation) # +proj=longlat +datum=WGS84 +no_defs

# Determine the number of grid cells in both raster objects
ncell(aarhus) #there is 11,115,556 grid cells
ncell(elevation) #there is 346,896 grid cells

#checking for units
aarhus
elevation #resolution = 0.008333333, 0.008333333  (x, y)
...
*Question 3: It makes sense that the extents and number of cells for Danish national elevation data and Aarhus city would be different because their sizes diverge, but why are the units so different? What are these units?*
  - for `aarhus` the units are m (meters)(?), and for `elevation` the units I can't find :(
  
  HOW TO FIND `elevation` UNITS? Is the units the same as the resolution??
...
plot(aarhus) #each layer are plottet seperately

# Plot the aarhus raster as an image
plotRGB(aarhus, stretch="linear")

# Plot the two images together
plot(aarhus);plotRGB(aarhus, add = TRUE)

plot(aarhus);plot(elevation, add=TRUE) 
#answer: no, the images can't be plottet together --> because they have different CRS's maybee?
...
st_crs(forests) # ETRS89 UTM 32N
st_crs(playgrounds) #there is no CRS

# Assign the CRS to playgrounds
crs_1 <- 4326
st_crs(playgrounds) <- crs_1

st_crs(playgrounds) #DID THIS WORK??? I can't see a difference


# Determine the CRS for the aarhus and elevation rasters
crs(aarhus, asText = TRUE) # +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs
crs(elevation, asText = TRUE) # +proj=longlat +ellps=WGS84 +no_defs

# Write here the proj4string for the aarhus layer
crs_2 <- "+proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs" 
crs(elevation) <- crs_2

crs(elevation) # the crs has changed!
...
the_crs <- crs(aarhus, asText = TRUE) #it is +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs
...
plotRGB(aarhus, alpha = 100, stretch = "lin")
plot(playgrounds, add = TRUE)

#I can't see the playgraounds. Therefore I set the crs to the same in aarhus and playgrounds. 
#transform CRS
# Get the CRS from the aarhus object
the_crs <- crs(aarhus, asText = TRUE) #it is +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs
...
# Again, plot aarhus and playgrounds (run both lines together)
plotRGB(aarhus, alpha = 100, stretch = "lin")
