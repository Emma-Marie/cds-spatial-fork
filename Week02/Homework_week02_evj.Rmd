...
tm_shape(nz_elev)  +
  ...
  tm_borders(col = "black", 
             lwd = 1) +
  tm_scale_bar(breaks = c(0, 150, 250),
               text.size = 1) +
  tm_compass(position = c("RIGHT", "top"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "E. V. Jørgensen, 14.02.2023") +
...
Inspecting the `zion` data: 
```{r inspect-data-1}
zion
head(zion)
# zion is a polygone. The CRS is UTM Zone 12, Northern Hemisphere. 
# The values of the coordinate system are xmin: 302903.1 ymin: 4112244 xmax: 334735.5 ymax: 4153087. 

class(zion)
# The zion data is a data frame and a tibble. 

st_crs(zion)
# The coordinate system used is GRS 1980. The datum is "unknown"
...
Inspecting the `srtm` data: 

```{r inspect-data-2}

# What can you say about the content of this file? 
srtm
# srtm is a Rasterlayer. Its CRS is  +proj=longlat +datum=WGS84 +no_defs. The dimensions of the raster is 457 rows and
# and 465 columns. 
...
Creating the object `srtm2` with the same crs as `zion`: 
```{r convert-crs-1}
...
# Project srtm2 to match the CRS of zion
srtm2_crs <- projectRaster(srtm2, crs = the_crs, method = "ngb")

# Look at the CRS to see if they match
st_crs(zion) # UTM Zone 12, Northern Hemisphere 
crs(srtm2_crs) # +proj=longlat +datum=WGS84 +no_defs 
#They don't match :((

#Setting the crs of srtm2 to the right crs
crs(srtm2) <- srtm2_crs
...
Setting the crs of `zion` to the same crs used in the `srtm` object:
