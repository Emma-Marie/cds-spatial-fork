...
class(viking) #it has a data fram

# Join the viking monuments with the municipalities
viking_neigh <- st_join(viking, municipalities_25832) # the two sets of data is combined into one data fram


# Confirm that viking_neigh has the municipality information
head(viking_neigh) #now it has 6 features (columns)

# Calculate the count of monuments per municipality
viking_neigh %>% 
  group_by(NAME_2) %>% 
  tally() %>% 
  arrange(desc(n)) #desc makes it go from the highest to lowest
...
plot(st_geometry(municipalities_25832$geometry), col = "grey", border = "white")
plot(municipalities_25832 %>% 
       filter(NAME_2 %in% least$NAME_2) %>% 
       st_geometry(), col = "red", add = T) 


#Adelas solution (which gives more red marks on the map)
plot(st_geometry(municipalities_25832$geometry), col = "grey", border = "white")
...
nb <- data.frame(place = "Nobelparken", longitude= 10.2033485, latitude=56.174346) #create data frame

nb_buf <- nb %>%
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>% #define the crs for R (before projection!)
  st_transform(crs = 25832) %>% # project to another crs
  st_buffer(10000)

# Check consistent projection among the objects
crs(nb_buf)==crs(viking)
crs(nb_buf)==crs(parks)
...
```
  ...
  - the object is a sgbp and also a list. 
  
2. Why do we use `parks[cont,]` to view the fully contained parks in the buffer plot?

- after finding all the parks contained in the buffer, we created a new column in the parks data set with the contained parks. This entire column (all he rows in it) we are calling with `parks[cont,]`???

3. How many parks and viking monuments respectively are there within the buffer?

```{r count-parks-and-vikings}
count(viking_clip) # 79
count(parks_clip) # 32
...
crs(viking_clip) == crs(nobel) # they do
...
sorted <- sort(d, decreasing = FALSE) # sort monuments by proximity
...
stenalder_monu <- monuments %>% 
  filter(monuments$datering == "Stenalder")

stenalder_monu

# Join the chosen monuments with the municipalities
stenalder <- st_join(stenalder_monu, municipalities_25832) # the two sets of data is combined into one data fram

head(stenalder)

#create (chloropleth?) map of Danish municipalities showing the density of the target monuments in a meaningful way. 


plot(st_geometry(municipalities_25832), border = "grey")
plot(stenalder_monu$geometry, add = TRUE, col = "blue", pch = 4)

#OBS NOT a chloropleth map - but Adelas solution doesn't work either :-(
```
