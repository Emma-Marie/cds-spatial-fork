...
leaflet() %>%
  ...
  setView(lng = 151.005006, lat = -33.971, zoom = 10) # sets what the map should be centered on
...
#not ethat all chunks are connected with pipes. 
...
# Set the location and zoom level
leaflet() %>% 
  setView(9.5155848, 55.9396761, zoom = 9) %>% # setting the center of the map as a centered place in Denmark. 
  addTiles()  # checking I am in the right area
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView(9.5155848, 55.9396761, zoom = 9)
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit#gid=1275868348",
                     range = "SA2022",
                     col_types = "cccnncnc") # the data type of the individual columns (characters and numbers)
glimpse(places)
```
...
# Set the location and zoom level
DKmap2 <- leaflet() %>% 
  setView(9.5155848, 55.9396761, zoom = 6) %>% # setting the center of the map as a centered place in Denmark. 
  addTiles() %>% 
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste0(places$Placename, "<br>",
...
```{r save-map}
library(htmlwidgets)
saveWidget(DKmap2, "DKmap2.html", selfcontained = TRUE)
```
