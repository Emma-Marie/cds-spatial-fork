...
highest25_mounds <- dplyr::slice_max(
  mounds, mounds$prominence,n=25,
  by = NULL,
  with_ties = TRUE,
  na_rm = FALSE
)
...
# Plot the sites
plot(elevation_prom_overlay);plot(sites, add = TRUE)

# Create a raster mask from the elevation_prom_overlay using the sites

#mask <- sites
#values(mask) <- sample(c(NA, TRUE), 36, replace = TRUE)
...
# extract the high elevation & prominence values for the `sites` object into `defensive` object
#defensive <- raster::extract(sites_strat, )

# Produce a list of the sites that occupy at least 6 grid cells of most prominent terrain
#sites[______(sapply(defensive, sum)>6),]
