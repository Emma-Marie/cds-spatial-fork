...
```{r rhumb}
distRhumb(my_points)  #this is a bit longer than the great cirkle distance, as it should be.
```
...
The longest is Rhumb and the shortest ist Meeus. 

```{r diff}
# Calculate the difference in km
(distRhumb(my_points)-distMeeus(my_points))/100 #7113.266 km

# Calculate percent difference
(distRhumb(my_points)-distMeeus(my_points))/distRhumb(my_points)*100 # gives 4,22%
```
...
# create geographic simple feature --> points on a globe (uses long/lat)
points_4326 <- points %>% 
st_as_sf(coords = c("longitude", "latitude"), crs = 4326) #changes from data frame to simple feature

# create a planimetric simple feature --> points in a plane (uses northing easting)
points_32632 <- points %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>% #the crs has to be set to the original crs before we can change it (because it started with no defined crs)
  st_transform(crs = 32632)

# calculate the Euclidian distance 
euc_dist <- st_distance(points_32632[1,],points_32632[2,]) # 22033412 meters --> 22033,412 km

# calculate geodetic distance
geo_dist <- st_distance(points_4326[1,],points_4326[2,]) # 16156702 meters --> 16156,702 km

# difference
euc_dist-geo_dist #5876710 meters --> 5876,710 km difference
(euc_dist-geo_dist)/geo_dist*100 #the difference is 36.37321 %

#this is why you don't use Euvclidian distance if you do long distance traveling!
```
