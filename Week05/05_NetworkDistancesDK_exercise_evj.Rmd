...
library(mapboxapi)
my_token <- "pk.eyJ1IjoiZW1tYW1hcmllIiwiYSI6ImNsZXdweHpjODJjbHkzcnJ2MHluYmk1YTIifQ.IH4-B3JGAFTHjyjvFCtL-w"
#my_token <- read_file("mytoken_evj.txt")
...
hjem <- mb_geocode("Ny Munkegade 73, Aarhus, 8000, Denmark ")
hjem

forældre <- mb_geocode("Vedbendvej 14, Risskov, 8240, Denmark ")
forældre
...
nobel_iso_walk <- mb_isochrone(nobel,
                                 profile = "walking",
                                 time = c(4, 8, 12))
...
leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data =nobel_iso_walk, 
              color = rev(colors),
              fillColor = rev(colors),
              fillOpacity = 0.5, 
              opacity = 0.5, 
...
```{r hjem-isochrone}
hjem_iso_walk <- mb_isochrone(hjem,
                                 profile = "walking",
                                 time = c(1:60))

hjem_iso_walk

pal <- colorNumeric("viridis", hjem_iso_walk$time, na.color = "transparent")

leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data =hjem_iso_walk, 
              color = rev(colors),
              fillColor = ~pal(time),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2) %>% 
  addLegend(values = hjem_iso_walk$time,
            pal = pal,
            title = "Walk-time<br/>around 'hjem'")

```

```{r route-hjem-nobel}
route <- mb_directions(origin = hjem,
                       destination = nobel,
                       profile = "cycling")
...
NB the map above is polygones. The map we creates next is rasters. 
...
iso_surface100 <- fasterize(isos_proj, template, field = "time", fun = "min")
...
mapbox_map %>%
  addRasterImage(iso_surface100, colors = pal, opacity = 0.5) %>%
...
Sys.setenv(OPENCAGE_KEY = "8f4e91d16e064498a5ae864d4d1de0d2")
...
```{r get-data}
Dk <- getData("GADM", country = "DNK", level=1)

view(Dk)
Midtjylland <- Dk[2,]

#projecting the data into 2D
Midtjylland <- st_as_sf(Midtjylland) %>% st_transform(25832)
driving_isos <- st_transform(driving_isos, 25832)
cycling_isos <- st_transform(cycling_isos, 25832)

#find the area that doesn't intersect with the driving polygons (st_difference())

outside_driving20 <- st_difference(st_union(driving_isos), Midtjylland)
library(mapview)
mapview(outside_driving20)

#isochromes: object <- st_area(Midt)-st_area(st_union(iso_driving20)) -->gives area i km2
(st_area(Midtjylland)-st_area(st_union(outside_driving20)))/1000000

#find the area that doesn't intersect with the cycling polygons (st_difference())

outside_cycling20 <- st_difference(st_union(cycling_isos), Midtjylland)
mapview(outside_cycling20)

#isochromes: object <- st_area(Midt)-st_area(st_union(iso_driving20)) -->gives area i km2
(st_area(Midtjylland)-st_area(st_union(outside_driving20)))/1000000 # 13023.96 km2 are not covered by the 20-min drive

(st_area(Midtjylland)-st_area(st_union(outside_cycling20)))/1000000 # 13123.27 km2 are not covered by the 20min bikeride. 
