...
date: "6-March-2023 updated `r Sys.Date()`"
output:
...
names(no_cars)[3:4] <- c("nc2020", "nc2021") 
```
...
#name the column V2 municipality in the "no_cars" data
names(no_cars)[2] <- "municipality" 
...
no_cars_pct_midt <- no_cars_pct_mun %>% 
  st_simplify(dTolerance = 100) %>% # reduces the weight
 # st_transform(4326) %>% 
...
#check that crs is 4326
st_crs(no_cars_pct_midt) #it says ID["EPSG",4326]

vulnerable_areas <- no_cars_pct_midt %>% 
  filter(nc2021pct >= 25) %>% #municipalities with over 25% of households not having a car
  st_difference(st_union(cycling_isos)) # only munis outside the 20-min cycling iso
...
mapbox_map %>%
  addPolygons(data = vulnerable_areas)
```
...
```{r lower-acces-cars}
area35pct <- no_cars_pct_midt %>% 
  filter(nc2021pct >= 35) %>% #municipalities with over 35% of households not having a car
  st_difference(st_union(cycling_isos)) # only munis outside the 20-min cycling iso


#check which areas

area35pct_size <- st_union(area35pct) # turns the separate areas into one
mapview(area35pct_size) # maps area
st_area(area35pct_size)/1000000 #find size of area in sq kms
...
mapbox_map %>%
  addPolygons(data = area35pct_size)

```

- In some areas 35% of the households or above have no aces to a car. This area is 442,2216 square kms. 


*4. How would you ensure residents in these areas have reasonable access to hospitals?*

- From the island Endelave there should be acces to a boat to sail sick people to the nearest hospital on the main land. The rest of the area should have good public transportation taking you to the nearest hospital. 
