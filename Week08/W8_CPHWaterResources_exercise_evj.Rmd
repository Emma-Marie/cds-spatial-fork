...
- Answer: The suburbs where organized slightly different for 100 years ago, the suburbs were smaller (e.g. todays Indre by was back then three different suburbs; indre by, Christianshavn and Voldkvartererne). 
...
#for each suburb in each year we get the total number of flats by suburb etc.
wcdata <- wc %>% 
  group_by(year, suburb_id) %>% 
  summarize(flats = sum(flats), #total number of flats by suburb
            bath = sum(bath), #total number of baths by suburb
            pct_bath = round(bath/flats*100,2), #percentage of flats with baths in a suburb
            wc_access= sum(wc_access),  #total number of toilets by suburb
            pct_wc= round(wc_access/flats*100,2), #percentage of flats with toilets in a suburb
            warmH20=sum(hot_water),    #total number of hot water taps by suburb
            pct_wH20=round(warmH20/flats*100,2),  # percentage of flats with hot water taps
...
- in 1970 (88.85749% had toilets)

```{r wc-bath-CPH, echo = FALSE}
# What percentage of flats have access to a bath in 1950 in Copenhagen as a whole?*
...
wc_spatial <- suburbs %>% 
  merge(wcdata, by.x= "id",by.y ="suburb_id") %>% 
    st_make_valid() #making data valid (because the polygones are a bit messy)
  
    
wc_spatial
sf::st_is_valid(wc_spatial) #checking if the data is valid - it is if the  st_make_valid() is in the script above. 
```
...
tm_shape(wc1950) +
  tm_borders(col = "black",
             lwd = 1) +
  tm_polygons("pct_bath", #you could also chose another thing e.g. pct_wc, flats etc.
              id = "navn",
             style = "pretty")+ # the style changes the way the data is grouped when plotted. COuld also be e.g. "jenks"
...
# Create a bounding box around the Copenhagen suburbs (we need to transform the data first)
...
assign("has_internet_via_proxy", TRUE, environment(curl::has_internet)) #tells R that you are in fact online (otherwise you get an error)
...
```{r osm-wrangle}
library(osmdata)
#remove duplicate geometries (they often appear in publicly reported data)
bath_uniq <- unique_osmdata(public_bath)

#filter out everything that isn't amenity, transform into WGC84 UTM32 and select the names
...
#intersecting the baths (points) and the suburbs (polygons)
baths_osm <- st_intersection(baths_osm, st_transform(suburbs, 32632) %>% st_geometry() %>% st_union())

# transform also historical baths -> the information about baths is attached to the centroid of each suburb.
...
```{r mapview-sync}
library(mapview)
library(leafsync) # can also be used if your computer don't like tmap
...
map_cph <-  mapview(baths_cph[,-3], 
        ...
        cex = "bath_per_km",
        legend = TRUE, 
        layer.name = "Baths per sq km <br>in suburbs from 1970", #you could also choose bath pr. suburb.
        homebutton = FALSE, lwd = 0.5) 
...
- the first is points and the others are suburbs, so it's 'apples' and 'oranges'. 
...
```
The map in the right is public bathrooms from the early 20th century, and the left is in the beginning of the 19th century (I canøt remember which year)
