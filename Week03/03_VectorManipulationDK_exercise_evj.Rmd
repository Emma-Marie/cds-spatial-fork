...
crs(shelters) #the EPSG is 25832 and is written at last as ID[\"EPSG\",25832]]
...
* Project/transform the `municipalities` (=Kommune) using the EPSG number 25832 as the target CRS. As the municipalities is a SpatialPolygons Dataframe, you will need to use `st_as_sf()` to convert it to `sf` object first.
...
municipalities <- readRDS("gadm36_DNK_2_sp.rds")
...
```{r second plot}
crs(aarhus) #crs is  +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs. The EPSG is 9001
st_crs(shelters, asText = TRUE) # ETRS89 / UTM zone 32N"

#So not the same CRS of aarhus and shelters. I set the CRS in shelters to the same as in aarhus.

crs_1 <- "+proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs"
st_crs(shelters) <- crs_1

st_crs(shelters)


# Second plot:  the `aarhus_box` and `shelters` on top of the aarhus ortophoto
plot(st_geometry(shelters), pch = 2)
plotRGB(aarhus, stretch = "lin", add = TRUE)
plot(st_geometry(aarhus_box), add = TRUE)
...
all.equal(____,_____) # use this for what??????


st_crs(shelters, asText = TRUE) # CRS is +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs (which I set it to above)
st_crs(aarhus_mun, asText = TRUE) # CRS is ETRS89 / UTM zone 32N

# projecting to shelters CRS if different
st_crs(aarhus_mun) <- crs_1
st_crs(aarhus_mun) # crs is now identical to crs of shelters

sh_buffer <- st_buffer(shelters, dist = 500)
...
length(sh_buffers) # length is 19
...
length(sh_buf_union) # length is 1
...
head(shelters) #the type of geometry is "POINT". 

# Convert the points to a single multi-point
sh1 <- st_union(shelters)

# Look at the data frame to see the type of geometry
head(sh1) # the type of geometry is "MULTIPOINT"

# Confirm that we went from 17 features to 1 feature
length(shelters) # length is 7
length(sh1) # length is 1
